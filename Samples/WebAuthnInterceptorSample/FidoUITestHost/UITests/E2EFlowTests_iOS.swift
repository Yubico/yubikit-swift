...
// iOS-specific E2E tests. The iOS ceremony shape is fundamentally different
// from macOS: `runCeremony` calls `collectPrefetchedPIN` upfront on iOS
// (Presenter+Ceremony.swift:50-52) so every ceremony begins with a PIN form,
// before any session is acquired. This file covers only the scenarios where
// that flow is coherent on iOS — i.e. real PIN-protected authenticators
// where typing a PIN upfront makes sense.
//
...
#if os(iOS)
final class E2EFlowTests_iOS: FidoUITestBase {
    ...
    /// the prefetch-PIN flow. Wired-flow coverage on iOS lives in
    /// `E2EFlowTests` (no launch arg, default `.wired`).
    ...
    func testAuthSubmitsPrefetchPIN() {
    ...
    /// returns immediately.
    func testAuthCancelsPrefetchPIN() {
