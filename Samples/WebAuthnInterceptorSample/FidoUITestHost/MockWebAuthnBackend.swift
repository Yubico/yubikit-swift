...
// Mirrors YubiKit/UnitTests/FIDO/WebAuthn/MockWebAuthnBackend.swift so scenarios
// can build WebAuthn.Client(backend: mock) via the internal init and exercise
// the real Client → Presenter → PanelView stack.
...
// `makeMockedFidoUI` lives in MockTransportController.swift now — it
// constructs FidoUI via the internal `_testTransportFactory` seam and a
// `MockTransportController` that owns the canned `ActiveSession`.
