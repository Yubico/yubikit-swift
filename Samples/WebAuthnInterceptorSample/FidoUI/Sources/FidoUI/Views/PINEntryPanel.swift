...
/// Inline PIN entry. The retry path lives entirely here: when
/// `config.errorMessage` flips to non-nil, the field shakes, clears, and
/// refocuses so the user knows their previous PIN was rejected.
struct PINEntryPanel: View {
    ...
    /// View-local shake counter. Increments when `config.errorMessage`
    /// transitions to non-nil — i.e. on a rejection retry. Lives here rather
    /// than the Presenter because it's strictly view-presentation state.
    ...
    var body: some View {
        ...
        .onChange(of: config.errorMessage) { _, newValue in
            // A new error means the prior PIN was rejected — clear the field,
            // shake, and refocus. Skip the shake when the message merely
            // clears (transition to nil), so we don't shake on the first
            // re-arm after cleanup.
