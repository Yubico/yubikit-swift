...
/// Initial fingerprint prompt — shown while the SDK is awaiting built-in UV.
/// `onCancel` calls the SDK keepalive cancel; `onUsePIN` (when non-nil) routes
/// the live ceremony into the PIN closure without restarting the connection.
...
/// Built-in UV exhausted; clientPin is configured so PIN is the path forward.
/// "Try Again" is intentionally absent — the authenticator's UV won't accept
/// a fingerprint until the key is reseated, so retrying inline is dead-ended.
...
/// Recovery prompt after a UV miss. Offers Try Again / Use PIN / Cancel.
