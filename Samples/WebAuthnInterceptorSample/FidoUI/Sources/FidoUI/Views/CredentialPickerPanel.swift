...
struct CredentialPickerPanel: View {
    ...
    /// Approximate row height (icon 36pt + padding 20pt + spacing). Used to
    /// grow the list to fit visible credentials before clamping to a scroll
    /// window for overflow.
    @ScaledMetric(relativeTo: .body) private var rowHeight: CGFloat = 62
    /// Cap above which the list scrolls instead of growing further.
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ScrollView {
                // Eager `VStack` rather than `LazyVStack`: with the typical
                // 1–10 credentials lazy loading buys nothing and SwiftUI's
                // lazy heuristic occasionally renders only the first row,
                // leaving the rest below the fold without scrolling them
                // into view.
                VStack(spacing: 6) {
