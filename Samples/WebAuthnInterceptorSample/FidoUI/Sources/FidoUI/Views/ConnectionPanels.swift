...
/// Shown both for the initial "no key yet" wait and for mid-ceremony
/// reconnect after a wired drop. `onCancel == nil` suppresses the
/// button on platforms with no usable cancel surface (iOS USB-C /
/// Lightning reconnect — user dismisses the sheet instead).
