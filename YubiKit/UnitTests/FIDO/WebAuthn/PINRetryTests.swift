...
struct PINRetryTests {
    ...
    @Test("pinBlocked CTAP error maps to pinBlocked")
    func testPinBlockedCTAPErrorMaps() async throws {
        let mock = MockWebAuthnBackend()
        mock.onGetInfo = { .stub(clientPin: true, pinUvAuthToken: true) }
        mock.onGetPinRetries = { .init(retries: 0, powerCycleState: false) }
        mock.onGetUVRetries = { 0 }
        mock.onGetPinUVToken = {
            ...
            throw CTAP2.SessionError.ctapError(.pinBlocked, source: .here())
        }
        mock.onGetAssertion = { _ in .mocked(.finished(.stub(credentialId: Data([0xAA])))) }
        ...
        var caught: WebAuthn.ClientError?
        do throws(WebAuthn.ClientError) {
            _ = try await client.getAssertion(Self.options, authorization: .pin("anything")).value()
        } catch {
            caught = error
        }
    ...
    @Test("Correct PIN drives the ceremony to success")
    func testCorrectPINSucceeds() async throws {
        let mock = MockWebAuthnBackend()
        mock.onGetInfo = { .stub(clientPin: true, pinUvAuthToken: true) }
        ...
        let stream = await client.getAssertion(Self.options, authorization: .pin("right"))
        var finished = false
        for try await status in stream {
            if case .finished = status { finished = true }
        }

        #expect(pinSubmissions == ["right"])
        #expect(finished)
    }

    // MARK: - Closure Behavior

    @Test("providePIN closure is invoked when a PIN is needed")
    func testProvidePINIsInvoked() async throws {
        let mock = MockWebAuthnBackend()
        mock.onGetInfo = { .stub(clientPin: true, pinUvAuthToken: true) }
        mock.onGetPinRetries = { .init(retries: 8, powerCycleState: false) }
        mock.onGetUVRetries = { 0 }
        ...
        #expect(asks.value == 1, "PIN closure should be invoked exactly once")
    }

    @Test("providePIN returning .cancel surfaces ClientError.cancelled")
    func testProvidePINCancelPropagates() async throws {
        let mock = MockWebAuthnBackend()
        mock.onGetInfo = { .stub(clientPin: true, pinUvAuthToken: true) }
        mock.onGetPinRetries = { .init(retries: 8, powerCycleState: false) }
        mock.onGetUVRetries = { 0 }
        mock.onGetPinUVToken = { _, _, _ throws(CTAP2.SessionError) in
            Issue.record("getPinUVToken should not be reached")
            throw CTAP2.SessionError.ctapError(.operationDenied, source: .here())
        }
        mock.onGetAssertion = { _ in .mocked(.finished(.stub(credentialId: Data([0xAA])))) }
        ...
        let auth = WebAuthn.Authorization(providePIN: { .cancel })
