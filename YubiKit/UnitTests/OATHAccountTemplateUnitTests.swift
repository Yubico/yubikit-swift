//
//  OATHAccountTemplateUnitTests.swift
//  YubiKitTests
//
//  Created by Jens Utbult on 2023-01-17.
//
...
final class OATHAccountTemplateUnitTests: XCTestCase {
    ...
    func testTOTPWithURL() throws {
        ...
        do {
            let template = try OATHSession.AccountTemplate(withURL: url)
            if case let OATHSession.AccountType.TOTP(period) = template.type {
                XCTAssert(period == 30)
            } else {
    ...
    func testTOTPSkipValidationWithURL() throws {
        ...
        do {
            let template = try OATHSession.AccountTemplate(withURL: url, skipValidation: true)
            if case let OATHSession.AccountType.TOTP(period) = template.type {
                XCTAssert(period == 30)
            } else {
                XCTFail("Wrong account type")
    ...
    func testHOTPWithURL() throws {
        ...
        do {
            let template = try OATHSession.AccountTemplate(withURL: url)
            if case let OATHSession.AccountType.HOTP(counter) = template.type {
    ...
    func testMissingSchemeWithURL() throws {
        ...
        do {
            _ = try OATHSession.AccountTemplate(withURL: url)
            XCTFail("Parsed url with wrong scheme.")
        } catch {
            if case OATHSession.AccountTemplateError.missingScheme = error {
    ...
    func testMalformedTypeWithURL() throws {
        ...
        do {
            ...
            if case OATHSession.AccountTemplateError.parseType = error {
                print("Function throwed expected error: \(error)")
            } else {
                XCTFail("Funktion throwed unexpeced error: \(error)")
            }
    ...
    func testMissingNameWithURL() throws {
        ...
        do {
            ...
            if case OATHSession.AccountTemplateError.missingName = error {
                print("Function throwed expected error: \(error)")
            } else {
                XCTFail("Funktion throwed unexpeced error: \(error)")
            }
    ...
    func testMissingSecretWithURL() throws {
        ...
        do {
            ...
            if case OATHSession.AccountTemplateError.missingSecret = error {
                print("Function throwed expected error: \(error)")
            } else {
                XCTFail("Funktion throwed unexpeced error: \(error)")
            }
    ...
    func testMalformedAlgorithmWithURL() throws {
        ...
        do {
            ...
            if case OATHSession.AccountTemplateError.parseAlgorithm = error {
                print("Function throwed expected error: \(error)")
            } else {
                XCTFail("Funktion throwed unexpeced error: \(error)")
            }
