...
public enum CompressionError: Error, Equatable {
    /// The data format is not recognized (neither Net iD nor gzip).
...
extension Data {
    ...
    /// Decompress certificate data using the appropriate method based on magic bytes.
    ///
    /// - Net iD format (0x01 0x00): Uses zlib decompression
    /// - Gzip format (0x1f 0x8b): Uses standard gzip decompression
    ///
    ...
    func decompressCertificate() throws(CompressionError) -> Data {
        if self.isNetId {
            do {
                return try self.netIdInflated()
            } catch {
                throw .decompressFailed("Net iD: \(error.message)")
            }
        } else if self.isGzipped {
            do {
