...
extension WebAuthn {
    ...
    public struct Authorization: Sendable {
        ...
        public enum UVPolicy: Sendable {
            /// Attempt built-in UV first; on failure fall through to the
            /// PIN closure when `clientPin` is configured. The default for
            /// most ceremonies.
            case preferred
            /// Skip built-in UV entirely and go straight to the PIN closure.
            case skipped
            /// Attempt built-in UV; on failure throw
            /// ``ClientError/uvBlocked(source:)`` rather than falling
            /// through to PIN. The PIN closure is never invoked.
            case required
        }
        ...
        /// Built-in UV only. The PIN closure is never invoked; on UV
        /// failure the ceremony throws ``ClientError/uvBlocked(source:)``.
