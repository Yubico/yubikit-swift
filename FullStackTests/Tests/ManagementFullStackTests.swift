...
struct ManagementFullStackTests {
    ...
    func disableAndEnableConfigOATHandPIVoverUSB() async throws {
        try await runManagementTest { connection, session, transport in
            ...
            #expect(!disabledInfo.config.isApplicationEnabled(.OATH, overTransport: .usb))
            #expect(!disabledInfo.config.isApplicationEnabled(.PIV, overTransport: .usb))
            ...
            #expect(enabledInfo.config.isApplicationEnabled(.OATH, overTransport: .usb))
            #expect(enabledInfo.config.isApplicationEnabled(.PIV, overTransport: .usb))
            #if os(iOS)
            await connection.nfcConnection?.close(message: "Test successful!")
            #endif
        }
    ...
    func disableAndEnableConfigOATHandPIVoverNFC() async throws {
        try await runManagementTest { connection, session, transport in
            ...
            try await session.updateDeviceConfig(disableConfig, reboot: false)
            let disabledInfo = try await session.getDeviceInfo()
            #expect(!disabledInfo.config.isApplicationEnabled(.OATH, overTransport: .nfc))
            #expect(!disabledInfo.config.isApplicationEnabled(.PIV, overTransport: .nfc))
            let oathSession = try? await OATHSession.session(withConnection: connection)
            if transport == .nfc {
                #expect(oathSession == nil)
            }
            ...
            try await managementSession.updateDeviceConfig(enableConfig, reboot: false)
            let enabledInfo = try await managementSession.getDeviceInfo()
            #expect(enabledInfo.config.isApplicationEnabled(.OATH, overTransport: .nfc))
            #expect(enabledInfo.config.isApplicationEnabled(.PIV, overTransport: .nfc))
            #if os(iOS)
            await connection.nfcConnection?.close(message: "Test successful!")
            #endif
        }
    ...
    func disableAndEnableWithHelperOATH() async throws {
        try await runManagementTest { connection, session, transport in
            ...
            #expect(!info.config.isApplicationEnabled(.OATH, overTransport: transport))
            let oathSession = try? await OATHSession.session(withConnection: connection)
            #expect(oathSession == nil)
            let managementSession = try await ManagementSession.session(withConnection: connection)
