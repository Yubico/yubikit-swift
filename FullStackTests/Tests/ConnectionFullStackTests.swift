...
struct ConnectionFullStackTests {
    ...
    @Test func singleConnection() async throws {
        let _ = try await Connection.connection()
        #expect(true, "Got connection")
    }

    @Test func serialConnections() async throws {
        let firstConnection = try await Connection.connection()
        #expect(true, "Got connection \(firstConnection)")
        let task = Task {
        ...
        #expect(closingError == nil)
        #expect(true, "connectionDidClose() returned: \(String(describing: closingError))")
    }

    @Test func connectionCancellation() async {
        let task1 = Task {
        ...
        #expect(true, "Result 1: \(String(describing: result1))")
        let result2 = try? await task2.value
        #expect(true, "Result 2: \(String(describing: result2))")
        let result3 = try? await task3.value
        #expect(true, "Result 3: \(String(describing: result3))")
        let result4 = try? await task4.value
        #expect(true, "Result 4: \(String(describing: result4))")

        #expect([result1, result2, result3, result4].compactMap { $0 }.count == 1)
    }

    @Test func sendManually() async throws {
        ...
        guard let versionData = records?.filter({ $0.tag == 0x05 }).first?.value else {
            Issue.record("No YubiKey version record in result.")
            return
        }
        guard versionData.count == 3 else {
            Issue.record("Wrong sized return data. Got \(versionData.hexEncodedString)")
            return
        }
        ...
        let minor = bytes[1]
        let micro = bytes[2]
        #expect(major == 5, "Got version: \(major).\(minor).\(micro)")
        ...
        let validStatuses: [ResponseStatus.StatusCode] = [.fileNotFound, .incorrectParameters, .invalidInstruction]
        #expect(
            validStatuses.contains(notFoundResult.responseStatus.status),
...
struct NFCFullStackTests {
    ...
    @Test func nfcAlertMessage() async throws {
    ...
    @Test func nfcClosingErrorMessage() async throws {
        let connection = try await TestableConnections.create(with: .nfc(alertMessage: "Test Alert Message"))
...
struct SmartCardConnectionFullStackTests {
    ...
    @Test func smartCardConnectionWithSlot() async throws {
